...
test('save to history, compare, and delete', async ({ page, request }) => {
    ...
    // The "Compare" step below must show THIS specific record's score, proving it
    ...
    await page.getByPlaceholder('Filter by project').fill(projectName);
    ...
    await seedRow.getByRole('button', { name: 'Compare' }).click();
...
test('a failed save shows an inline error without breaking the results page', async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'Save to history' }).click();
    await expect(
        page.getByText('Unable to save to history. Please try again.')
    ).toBeVisible();
