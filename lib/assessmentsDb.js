...
function createConnection() {
    ...
    `);
    const existingColumns = connection.prepare('PRAGMA table_info(assessments)').all().map((col) => col.name);
    if (!existingColumns.includes('gap_analysis_report')) {
        connection.exec('ALTER TABLE assessments ADD COLUMN gap_analysis_report TEXT');
    }
    return connection;
}
